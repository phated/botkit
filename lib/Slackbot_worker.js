...
module.exports = function(botkit, config) {
    ...
    bot.replyPublic = function(src, resp, cb) {
        if (!bot.res) {
            botkit.debug('No web response object found. Falling back to delayed reply.');
            bot.replyPublicDelayed(src, resp, cb);
        } else {
            var msg = {};
    ...
    bot.replyPrivate = function(src, resp, cb) {
        if (!bot.res) {
            botkit.debug('No web response object found. Falling back to delayed reply.');
            bot.replyPrivateDelayed(src, resp, cb);
        } else {
            var msg = {};
    ...
    bot.joinChannel = function(channelId, cb) {
        if (!channelId) {
            cb && cb(new Error('No channel ID specified.'));
        } else {
            var botId = bot.config.bot.user_id;
            bot.api.channels.invite({ user: botId, channel: channelId }, cb);
        }
    }
